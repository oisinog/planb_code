...
cell_indices <- raster::extract(final_raster_stack_41_70, xy)
...
train_data_values <- extract(final_raster_stack_41_70, occtrain)
...
pseudo_absence_values <- extract(final_raster_stack_41_70, pseudo_absences)
...
write.csv(results_df, file = "esm_optimal_thresholds_tss_41_70.csv", row.names = FALSE)

# Plot TSS values against thresholds
pdf("esm_TSS_vs_Threshold_plot_41_70.pdf")  # Open a PDF file to save the plot
plot(thresholds, tss_values, type = "l", col = "blue", lwd = 2,
     xlab = "Threshold", ylab = "TSS", main = "TSS vs Threshold (2041-2070)")
