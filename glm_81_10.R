...
set.seed(123)
pa_81_10 <- rbind(as.data.frame(p_81_10), as.data.frame(a_81_10))
pa_81_10$pa <- as.factor(c(rep(1, nrow(p_81_10)), rep(0, nrow(a_81_10))))
colnames(pa_81_10) <- c(paste0("bio", 1:19), "slope", "SurficialGeology", "TWI", "aspect", "pa")
# Save the pa_81_10 data to a CSV file
write.csv(pa_81_10, "pa_81_10.csv", row.names = FALSE)
xdata <- pa_81_10[,1:23] #X is automatically standardized when input into the model!!!
ydata <- pa_81_10[,24]
...
data.scaled_81_10 <- cbind(x.scaled, SurficialGeology = xdata$SurficialGeology, TWI = xdata$TWI, pa = ydata)

year_model_81_10 <- glm(ydata ~ bio9 + bio14
                        + SurficialGeology + TWI + aspect,
                        data = data.scaled_81_10, family = binomial(link="logit"))
summary(year_model_81_10)
vif(year_model_81_10)

seasonal_model_81_10 <- glm(ydata ~ bio15
                            + bio4 + TWI + slope 
                            + aspect + I(aspect^2),
                            data = data.scaled_81_10, family = binomial(link="logit"))
summary(seasonal_model_81_10)
vif(seasonal_model_81_10)

annual_model_81_10 <- glm(ydata ~ bio1 + I(bio1^2) + bio7 + 
                            TWI + slope,
                          data = data.scaled_81_10, family = binomial(link="logit"))
summary(annual_model_81_10)
vif(annual_model_81_10)

winter_model_81_10 <- glm(ydata ~ bio11 + I(bio11^2) + TWI + I(TWI^2) + slope 
                          + aspect,
                          data = data.scaled_81_10, family = binomial(link="logit"))
summary(winter_model_81_10)
vif(winter_model_81_10)

drought_model_81_10 <- glm(ydata ~ bio13 + bio14 
                           + TWI + slope + I(slope^2) + aspect + I(aspect^2),
                           data = data.scaled_81_10, family = binomial(link="logit"))
summary(drought_model_81_10)
vif(drought_model_81_10)

cold_model_81_10 <- glm(ydata ~ bio13 + bio6
                        + bio7 + TWI + slope + aspect 
                        + I(aspect^2),
                        data = data.scaled_81_10, family = binomial(link="logit"))
summary(cold_model_81_10)
vif(cold_model_81_10)

extremes_model_81_10 <- glm(ydata ~ bio17 
                            + bio7 + TWI + slope
                            + aspect + I(aspect^2),
                            data = data.scaled_81_10, family = binomial(link="logit"))
summary(extremes_model_81_10)
vif(extremes_model_81_10)

# TSS Calculations for GLM models

glm_models <- list(
  "Year Model (1981-2010)" = year_model_81_10,
  "Seasonal Model (1981-2010)" = seasonal_model_81_10,
  "Annual Model (1981-2010)" = annual_model_81_10,
  "Winter Model (1981-2010)" = winter_model_81_10,
  "Drought Model (1981-2010)" = drought_model_81_10,
  "Cold Model (1981-2010)" = cold_model_81_10,
  "Extremes Model (1981-2010)" = extremes_model_81_10
)
...
results <- lapply(glm_models, function(model) {
  predictions <- predict(model, data.scaled_81_10, type = "response")
  actual <- data.scaled_81_10$ydata
  
  thresholds <- seq(0, 1, by = 0.01)
  tss_values <- sapply(thresholds, function(threshold) calculate_tss(model, data.scaled_81_10, threshold))
...
write.csv(results_df, "glm_tss_results_81_10.csv", row.names = FALSE)

# Plot TSS values against thresholds for each model and save plots
pdf("glm_tss_plots_81_10.pdf")
...
writeLines(output_summary, "glm_model_summaries_81_10.txt")
